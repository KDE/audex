...
void CDTextReaderTask::run()
{
    ...
    if (!tracks.value(0).value(0x80).simplified().isEmpty())
        cdtext.set(Metadata::Title, tracks.value(0).value(0x80)); // TITLE
    ...
    Q_EMIT finished(drive_udi, true, cdtext);
}

void CDTextReaderTask::debug_raw_text_pack_data(const QByteArray &data)
{
