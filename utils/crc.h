...
#define CRC_FILE_BUFSIZE 128 * 1024 // 128 KiB

static const quint16 x25_table[1 << 8] = {
...
class CRC16_X25_Calculator
{
public:
    CRC16_X25_Calculator(const quint16 initialValue = 0);

    quint16 result() const;
    QString resultHexString() const;

    void reset(const quint16 initialValue = 0);

    void process(const QByteArray &data);
    bool processFile(const QString &filename);

    static quint16 calc(const QByteArray &data, const quint16 initialValue = 0);
...
#define CRC32_MASK1 0x00FFFFFF
#define CRC32_MASK2 0xFFFFFFFF

class CRC32_Calculator
{
public:
    CRC32_Calculator(const quint32 initialValue = -1);

    quint32 result() const;
    QString resultHexString() const;

    void reset(const quint32 initialValue = -1);

    void process(const QByteArray &data);
    bool processFile(const QString &filename);

    static quint32 calc(const QByteArray &data, const quint32 initialValue = -1);

private:
    quint32 crc;
};

class CRC32_PCM_Calculator
{
public:
    CRC32_PCM_Calculator(const quint32 initialValue = 0xFFFFFFFF);

    quint32 result() const;
    QString resultHexString() const;

    void reset(const quint32 initialValue = 0xFFFFFFFF);

    void process(const QByteArray &data);

    static quint32 calc(const QByteArray &data, const quint32 initialValue = 0xFFFFFFFF);
